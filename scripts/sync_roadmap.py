...
class RoadmapManager:
    def __init__(self, github_token: str, dry_run: bool = False):
        ...
        project_details = self._get_project_details_by_number(
            CONFIG["ROADMAP_PROJECT_ORG"], CONFIG["ROADMAP_PROJECT_ID"], owner_type="user"
        )
        if not project_details:
            raise ValueError(f"Could not find roadmap project with ID {CONFIG['ROADMAP_PROJECT_ID']}")
        self.roadmap_project_node_id = project_details["id"]
        self.roadmap_repo = self.github_client.get_repo(CONFIG["ROADMAP_REPO"])
    ...
    def get_sigs_projects(self) -> dict[str, list[dict]]:
        ...
        repo = self.github_client.get_repo(CONFIG["COMMUNITY_REPO"])
        content = repo.get_contents(CONFIG["SIGS_FILE"], ref="add_roadmap_management")
